...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/home/anuj/Desktop/Testings/key.json"
...
def pdf_generator(class_name, subject,chapter,query):
    ...
    if query=="no-input-recieved":
        pdf_file_path = f"/home/anuj/Desktop/Testings/Books/{class_name}/{subject}/{chapter}.pdf"
        pdf_text = sumerize.summarize_text(extract_text_from_pdf(pdf_file_path))
        study_material = generate_study_material(pdf_text,subject,query)
        print("study material==",study_material,end='\n')
        pdf_filename = generate_pdf(class_name, subject, chapter, study_material,query)
    ...
    for point in points:
        ...
        pdf_filename = generate_pdf(class_name, subject, chapter, study_material,point)
        pdf_paths.append(pdf_filename)
    return pdf_paths
...
input_components = [
    gr.inputs.Dropdown(["Class 10", "Class 11", "Class 12"], label="Select Class"),
    gr.inputs.Dropdown(["Math", "Science", "English","Chemistry"], label="Select Subject"),
    gr.inputs.Dropdown(["Chapter 1", "Chapter 2", "Chapter 3"], label="Select Chapters"),
    gr.inputs.Textbox(label="Write the topics name saperated by ','",default="no-input-recieved")
]
